...
code, which is built on czyzby's code, gives us working projects that use Gradle 6.7, the same as the official setup, but ahead of 4.0.2 for czyzby's gdx-setup.
Currently, gdx-liftoff projects depend on libGDX 1.9.12 by default, and allow using snapshots as well.
...
## Usage
  ...
  - Run the JAR. Plug in whatever options you see fit:
    - For the Platforms tab, LWJGL3 starts enabled by default (it works on all desktop/laptop platforms), and you can
      ...
      - Desktop and/or LWJGL3 should usually be checked, so you can test on the same computer you develop on.
        - LWJGL3 is almost the same as Desktop, but because it has better support for new hardware
          ...
          - LWJGL3 itself supports Linux on arm32 and arm64 hardware, and the current 1.9.12 release of libGDX also
        ...
        - Desktop should mostly be preferred if you need to also depend on gdx-tools, such as if you need to run the
          ...
          - The framerate limit problem is currently solved with both Desktop and LWJGL3, as long as you use libGDX 1.9.12.
      ...
      - Android should only be checked if you've set up your computer for Android development. Since gdx-liftoff uses
        Gradle 6.7, having an Android project present shouldn't interfere with other platforms or IDE integration, as
        long as your IDE supports Gradle 6.7 (Android Studio may in its most recent versions, but IntelliJ IDEA (and
      ...
      - HTML is a more-involved target, with some perfectly-normal code on all other platforms acting completely
        ...
        - GWT 2.9.0 is available but doesn't integrate with libGDX by default; there's a third-party [replacement to the
          ...
          - The GWT 2.9.0 support isn't yet working with libGDX 1.9.12, and gdx-liftoff defaults to GWT 2.8.2 . 
    ...
    - In Advanced, you can set the libGDX version (it defaults to 1.9.12, but can be set lower or higher) and
...
see [libGDX's documentation](https://libgdx.badlogicgames.com/documentation/gettingstarted/Importing%20into%20IDE.html).
  ...
  - If you had the GWT option checked in the setup and have a non-empty template,
    ...
    task for the `gwt` module, or also possibly the `dist` task in that module.
      - GWT builds have gotten much faster with Gradle 6.7 and some adjustments to configuration, so
  ...
  - All builds currently use Gradle 6.7 with the "api/implementation/compile fiasco" resolved. Adding dependencies
