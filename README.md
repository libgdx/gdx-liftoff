...
code, which is built on czyzby's code, gives us working projects that use Gradle 6.6, instead of 5.4.1 for the official setup or 4.0.2 for czyzby's gdx-setup.
    ...
    - For the Platforms tab, you can use the "Toggle Commonly-Used Platforms" button to enable or disable
      ...
      - Desktop and/or LWJGL3 should usually be checked, so you can test on the same computer
        ...
        - LWJGL3 is almost the same as Desktop, but because it has better support for new hardware
          ...
          - Liftoff customizes the LWJGL3 dependency to match what libGDX can use. Projects made with gdx-liftoff won't
            depend on ARM Linux native libraries, since libGDX can't use them, and won't include them in releases. It's
            easy to change this behavior if libGDX does start to support ARM.
      ...
      - Android should only be checked if you've set up your computer for Android development. Unlike with some other
        setup tools, since gdx-liftoff uses Gradle 6.6, having an Android project present shouldn't interfere with
        other platforms or IDE integration, as long as your IDE supports Gradle 6.6 (Android Studio may in its most
...
see [libGDX's documentation](https://libgdx.badlogicgames.com/documentation/gettingstarted/Importing%20into%20IDE.html).
  ...
  - If you had the GWT option checked in the setup and have a non-empty template,
    ...
    task for the `gwt` module, or also possibly the `dist` task in that module.
      - GWT builds have gotten much faster with Gradle 6.6 and some adjustments to configuration, so
  ...
  - All builds currently use Gradle 6.6 with the "api/implementation/compile fiasco" resolved. Adding dependencies
