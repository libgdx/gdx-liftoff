...
enough that they won't work with Java 13 or newer, and neither seems to be updated very often.
...
code, which is built on czyzby's code, gives us working projects that use Gradle 6.7, instead of 5.4.1 for the official setup or 4.0.2 for czyzby's gdx-setup.
    ...
    - For the Platforms tab, you can use the "Toggle Commonly-Used Platforms" button to enable or disable
      ...
      - Desktop and/or LWJGL3 should usually be checked, so you can test on the same computer
        ...
        - LWJGL3 is almost the same as Desktop, but because it has better support for new hardware
          ...
          - LWJGL3 itself supports Linux on arm32 and arm64 hardware, but libGDX does not (in the current stable
            version). The upcoming 1.10.0 release is expected to support this Linux hardware, and the 1.9.12-SNAPSHOT
            release should support it now.
        - Desktop should mostly be preferred if you need to also depend on gdx-tools, such as if you need to run the
          ...
          - The framerate limit problem is currently only solved easily with the Desktop platform, but it can be
            ...
            of libGDX will do this for you, and 1.9.12-SNAPSHOT already does.
      ...
      - Android should only be checked if you've set up your computer for Android development. Unlike with some other
        setup tools, since gdx-liftoff uses Gradle 6.7, having an Android project present shouldn't interfere with
        other platforms or IDE integration, as long as your IDE supports Gradle 6.7 (Android Studio may in its most
