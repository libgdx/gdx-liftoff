...
code, which is built on czyzby's code, gives us working projects that use Gradle 6.5, instead of 5.4.1 for the official setup or 4.0.2 for czyzby's gdx-setup.
  ...
  - Run the JAR. Plug in whatever options you see fit:
    - For the Platforms tab, you can use the "Toggle Commonly-Used Platforms" button to enable or disable
      ...
      - Android should only be checked if you've set up your computer for Android development. Unlike with some other
        setup tools, since gdx-liftoff uses Gradle 6.5, having an Android project present shouldn't interfere with
        other platforms or IDE integration, as long as your IDE supports Gradle 6.5 (Android Studio does not except
        for its beta or canary builds of 4.0.0 or a higher Android Studio version).
      - HTML is a more-involved target, with some perfectly-normal code on all other platforms acting completely
        ...
        - GWT 2.9.0 is available but doesn't integrate with libGDX by default; there's a third-party [replacement to the
          official GWT backend](https://github.com/tommyettinger/gdx-backends#19102) that supports it with libGDX 1.9.10
          and [a fork of that backend](https://github.com/raeleus/gdx-backends-jdk11/tree/update-to-gdx-11-snapshot)
          that changes the libGDX version to 1.9.11-SNAPSHOT. Using GWT 2.9.0 allows Java 11's `var` keyword to be used,
          plus other Java 11 features, but doesn't change much of what's available from the standard library.
...
see [libGDX's documentation](https://libgdx.badlogicgames.com/documentation/gettingstarted/Importing%20into%20IDE.html).
  ...
  - If you had the GWT option checked in the setup and have a non-empty template,
    ...
    task for the `gwt` module, or also possibly the `dist` task in that module.
      - GWT builds have gotten much faster with Gradle 6.5 and some adjustments to configuration, so
  ...
  - All builds currently use Gradle 6.5 with the "api/implementation/compile fiasco" resolved. Adding dependencies
