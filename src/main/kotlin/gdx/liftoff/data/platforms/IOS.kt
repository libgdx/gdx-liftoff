...
class IOSGradleFile(
  ...
  init {
    ...
    // The check for greater than 1.14.1 assumes https://github.com/libgdx/libgdx/issues/7422 gets merged.
    addDependency(
      "com.badlogicgames.gdx:gdx-backend-robovm${if (GdxVersion.parseGdxVersion(project.advanced.gdxVersion) != null &&
        (
          GdxVersion.parseGdxVersion(project.advanced.gdxVersion)!! < GdxVersion(1, 12, 0) ||
            GdxVersion.parseGdxVersion(project.advanced.gdxVersion)!! > GdxVersion(1, 14, 1)
        )
      ) {
        ""
      } else {
