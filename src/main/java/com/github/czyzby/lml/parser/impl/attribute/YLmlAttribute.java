 ...
 * position) or a float ending with a '%' (for example, "0.5%"). If a percent is passed, it will set the Y position as a
 * percent of stage's initial height. For the most predictable results, set both x and y attributes.
 *
 * <p>
 * By default, mapped to "y".
 *
 * @author MJ
 * @see XLmlAttribute */
public class YLmlAttribute implements LmlAttribute<Actor> {
    ...
    public void process(final LmlParser parser, final LmlTag tag, final Actor actor, final String rawAttributeData) {
        if (tag.getParent() == null) { // Attaching to stage with selected position:
            ...
            actor.setY(parser.parseFloat(rawAttributeData, actor));
        }
    }
}
