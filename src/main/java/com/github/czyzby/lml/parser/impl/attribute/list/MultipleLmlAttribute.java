...
/** See {@link com.badlogic.gdx.scenes.scene2d.utils.Selection#setMultiple(boolean)}. Mapped to "multiple".
 *
 * @author MJ */
public class MultipleLmlAttribute implements LmlAttribute<List<?>> {
    @Override
    ...
    public void process(final LmlParser parser, final LmlTag tag, final List<?> actor, final String rawAttributeData) {
        actor.getSelection().setMultiple(parser.parseBoolean(rawAttributeData, actor));
    }
}
