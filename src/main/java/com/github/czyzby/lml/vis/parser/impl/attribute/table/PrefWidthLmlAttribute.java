...
/** See {@link Cell#prefWidth(Value)}. See {@link LmlUtilities#parseHorizontalValue(LmlParser, LmlTag, Actor, String)}
 * and {@link LmlUtilities#parseVerticalValue(LmlParser, LmlTag, Actor, String)} for more info on value parsing. Honors
 * {@link FloatingGroup} settings. Mapped to "prefWidth".
 *
 * @author MJ */
public class PrefWidthLmlAttribute extends AbstractCellLmlAttribute {
    @Override
    public void process(final LmlParser parser, final LmlTag tag, final Actor actor, final Cell<?> cell,
        ...
        final Value horizontalValue = LmlUtilities.parseHorizontalValue(parser, tag.getParent(), actor,
                rawAttributeData);
        cell.prefWidth(horizontalValue);
    ...
    protected void processForActor(final LmlParser parser, final LmlTag tag, final Actor actor,
            final String rawAttributeData) {
        if (actor instanceof FloatingGroup) {
            ((FloatingGroup) actor).setPrefWidth(parser.parseFloat(rawAttributeData, actor));
        } else {
