...
Currently, gdx-liftoff projects depend on libGDX 1.14.1 by default, and allow using earlier versions or snapshots as well.
The current version of libGDX is 1.14.1, which came out about 7 months after 1.14.0 (which is essentially fine) and a
year after 1.13.5 (which has some significant issues). 1.13.1 was the (working) version before that. Don't use 1.13.0,
or any of the partial releases 1.13.2, 1.13.3, or 1.13.4; the releases 1.14.1, 1.14.0 and 1.13.1 are good and should be
...
- Run the JAR. Plug in whatever options you see fit:
            ...
            - LWJGL3 is almost the same as LWJGL2, but because it has better support for new hardware
              (such as high-DPI displays), it should probably be preferred. It also allows multiple windows and drag+drop.
                - LWJGL3 itself supports Linux on arm32 and arm64 hardware, and libGDX since version 1.9.13 (current is
                  1.14.1) also supports ARM Linux on desktop platforms.
    ...
    - In Advanced, you can set the libGDX version (it defaults to 1.14.1, but can be set lower or higher) and
